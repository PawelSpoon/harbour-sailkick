...
SilicaListView {
    ...
    function fillUpCommingModelForOneTrackingEntry(type, events)
    {
        ...
        for (var i = 0; i < events.length; i++)
        {
            ...
            upcomingModel.append({"title": shortTitle, "type": events[i].metroAreaName, "venue": events[i].venueName ,"date": dateWithDay(events[i].date), "uri" : events[i].eventUrl, "attendance": events[i].attendance,
