...
class SongkickScraper:
    ...
    def get_location_events(self, location_id):
        ...
        for event in events:
            try:
                # Parse JSON-LD data which contains structured event information
                import json
                event_data = json.loads(event.string)
                if event_data.get('@type') == 'Event':
                    # Handle performer data which can be either a dict, list, or None
                    performer = event_data.get('performer', [])
                    if isinstance(performer, dict):
                        performer_name = performer.get('name', 'N/A')
                    elif isinstance(performer, list) and performer:
                        performer_name = ', '.join(p.get('name', 'N/A') for p in performer)
                    else:
                        performer_name = 'N/A'
                        
                    results.append({
                        'name': event_data.get('name', 'N/A'),
                        'date': event_data.get('startDate', 'N/A'),
                        'venue': event_data.get('location', {}).get('name', 'N/A'),
